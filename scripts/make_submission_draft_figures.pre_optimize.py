...
OUT=ROOT/'outputs'/'submission-draft-figures'; OUT.mkdir(parents=True,exist_ok=True)
BLUE='#2F6B9A'; TEAL='#2A9D8F'; ORANGE='#E6953B'; RED='#C94C4C'; NAVY='#17324D'; GREY='#75808A'; LIGHT='#E8EEF2'
...
def panel(ax,label): ax.text(-.12,1.08,label,transform=ax.transAxes,fontsize=12,fontweight='bold',color=NAVY,va='top')
def save(fig,name):
    fig.savefig(OUT/f'{name}.png',dpi=120,bbox_inches='tight',facecolor='white')
...
def figure1():
    fig,ax=plt.subplots(figsize=(11,6.5)); ax.axis('off'); ax.set_xlim(0,1); ax.set_ylim(0,1)
    ...
    notes=['GSE53408 secondary lung cohort: g = 1.08','GSE210248: PDE8B localized to SMC1/SMC2','GSE293580: IPAH g = 1.33; SSc-PAH g = 0.72']
    for i,t in enumerate(notes): ax.text(.07,.20-i*.055,u'\u2022 '+t,fontsize=9)
    ax.text(.60,.20,'Core interpretation',fontweight='bold',color=NAVY,fontsize=10)
    ax.text(.62,.15,'PDE8B marks a smooth-muscle-enriched\ncyclic-nucleotide/remodeling state,\nnot a proven causal regulator.',fontsize=10,color=RED)
...
def figure2():
    ...
    axs[0].set_yticks(y,names); axs[0].set_xlabel('Hedges g (PAH vs donor)'); axs[0].set_title('PDE8B across lung cohorts',fontweight='bold'); axs[0].text(.03,-.22,'Meta g=0.74 (95% CI 0.47–1.02); I²=0%',transform=axs[0].transAxes,color=RED)
...
def figure3():
    u=pd.read_csv(ROOT/'processed'/'gse210248_umap'/'gse210248_recomputed_umap.csv.gz'); prop=pd.read_csv(COEX/'state-deconvolution'/'gse210248_celltype_proportions.csv')
    ...
    axs[0].set_title('Cell-type landscape'); axs[0].set(xticks=[],yticks=[],xlabel='UMAP 1',ylabel='UMAP 2'); panel(axs[0],'A')
    ...
    axs[1].set_title('PDE8B expression'); axs[1].set(xticks=[],yticks=[],xlabel='UMAP 1',ylabel='UMAP 2'); panel(axs[1],'B')
...
def figure4():
    ...
    ax=fig.add_subplot(gs[:,0]); panel(ax,'A'); names=['PDE8B','contractile','synthetic_ecm','mural_remodeling']; lab=['PDE8B','Contractile','Synthetic/ECM','Mural remodeling']; y=np.arange(4)
    ...
    ax.axvline(0,color=GREY,lw=.8); ax.set_yticks(y,lab); ax.invert_yaxis(); ax.set_xlabel('Hedges g (PAH vs donor)'); ax.set_title('SMC state modules',fontweight='bold'); ax.legend(frameon=False)
    ...
    for x0,title,sub,col in [(.03,'Mature contractile SMC','ACTA2 TAGLN MYH11\nCNN1 MYL9 TPM2',BLUE),(.63,'Remodeling SMC','PDE8B COL3A1 FN1\nVIM / ECM program',RED)]:
    ...
    ax2.add_patch(FancyArrowPatch((.39,.51),(.61,.51),arrowstyle='simple',mutation_scale=18,color=ORANGE,alpha=.85)); ax2.text(.50,.64,'PAH state transition',ha='center',fontweight='bold',color=ORANGE)
    ...
    ax3.bar(q.cell_type,q.hedges_g,color=[BLUE,RED]); ax3.axhline(0,color=GREY,lw=.8); ax3.set_ylabel('Hedges g for cell fraction'); ax3.set_title('SMC abundance shift',fontweight='bold'); ax3.text(.02,-.28,'n=3 PAH and 3 donors; descriptive effect sizes',transform=ax3.transAxes,color=GREY)
    fig.suptitle('Figure 4. PDE8B upregulation accompanies loss of the mature SMC contractile state',fontsize=14,fontweight='bold',color=NAVY); save(fig,'Figure_4_SMC_state_transition')
...
def figure5():
    ...
    axs[0].set(xlabel='Meta-correlation with PDE8B',ylabel='−log10(FDR)',title='Cross-cohort co-expression core')
    ...
    axs[2].axhline(0,color=GREY,lw=.8); axs[2].set_xticks(xx,['PDE8B','Positive\ncore','Contractile/\ncyclic','Signed\nfull'],rotation=25); axs[2].set_ylabel('Hedges g'); axs[2].set_title('Independent GSE293580'); axs[2].legend(frameon=False)
